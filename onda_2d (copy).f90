...
real function source(A,pi,fc,k,dt,t0) 
implicit none
integer :: k
real :: A, pi, fc, dt, t0

source=A*(2*pi*(pi*fc*(k*dt-t0))**2-1)*exp(-pi*(pi*fc*(k*dt-t0))**2)
...
!staggered_grid_source=-2*A*(k*dt-t0)*exp(-A*(k*dt-t0)**2)

end function staggered_grid_source

!#######################################################

subroutine second_order_operator_loop()
implicit none

do j=2, Nx-1
    do i=2, Nz-1            
            P3(i,j)=(delta*delta*(mod_vel(i,j)*mod_vel(i,j)))*(P2(i+1,j)+P2(i-1,j)+P2(i,j+1)+P2(i,j-1)-4*P2(i,j))+2*P2(i,j)-P1(i,j)
    end do
end do

end subroutine second_order_operator_loop


!#######################################################

subroutine second_order_operator_loop_2()
implicit none
...
end subroutine second_order_operator_loop_2
...
subroutine fourth_order_operator_loop()
implicit none

 do j=3, Nx-2
        do i=3, Nz-2            
            P3(i,j)=(delta*delta*mod_vel(i,j)*mod_vel(i,j)/12)*(-P2(i-2,j)-P2(i,j-2)-P2(i+2,j)-P2(i,j+2)+ &
            & 16*(P2(i-1,j)+P2(i,j-1)+P2(i+1,j)+P2(i,j+1))-60*P2(i,j))+2*P2(i,j)-P1(i,j)
        end do
    end do

end subroutine fourth_order_operator_loop
